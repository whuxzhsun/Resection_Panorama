...
int SPP_S::solvePanoParameter(panoPara &pp, std::vector<pointData> pd)
{
	...
	do 
	{
		...
		/**************************************************/
		Matrix v = B * x - L;
/*		double meanErrorX, meanErrorY;
		double sumErrorX, sumErrorY;
		sumErrorX = sumErrorY = 0;
		for (i = 0; i < n; i++)
		{
			sumErrorX -= fabs(v(i * 2 + 0, 0));
			sumErrorY -= fabs(v(i * 2 + 1, 0));
		}
		meanErrorX = sumErrorX / n;
		meanErrorY = sumErrorY / n;
		outDebug << "删除粗差点：" << endl;
		for (i = 0, j = 0; i < pd.size() && k < 0; i++, j++)
		{
			if (fabs(v(j * 2 + 0, 0)) > 2 * meanErrorX || fabs(v(j * 2 + 1, 0)) > 2 * meanErrorY)
			{
				outDebug << pd[i].px << "\t" << pd[i].py << "\t" << v(j * 2 + 0, 0) << "\t" << v(j * 2 + 1, 0) << endl;
				pd.erase(pd.begin() + i);
				i--;
			}
		}//*/
