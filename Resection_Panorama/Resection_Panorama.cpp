...
//	outDiff << "diff_x		diff_y		diff_z		diff_roll	diff_pitch	diff_yaw\n";
	...
	panoPara truePP;
	truePP.xs = 458317.6595;
	truePP.ys = 4403414.6751;
	truePP.zs = 23.3344;
	truePP.alpha = 0.015048;
	truePP.phi =   0.053347;
	truePP.belta = 3.173651;

	double pixelToAngle = 360.0 / 8192;
/*
	SPP_S spp(8192, 4096);
	spp.solvePanoParameter_ceres2(pp, pd);
	...
	for (int i = 0; i < nPts; i++)
	{
		...
		for (int j = i + 1; j < nPts; j++)
		{
			...
			for (int k = j + 1; k < nPts; k++)
			{
				...
				SPP_S spp(8192, 4096);
				spp.solvePanoParameter_ceres2(pp, pd2);
		...
		for (int j = i + 1; j < nPts; j++)
		{
			...
			for (int k = j + 1; k < nPts; k++)
			{
				...
				for (int l = k + 1; l < nPts; l++)
				{
					...
					SPP_S spp(8192, 4096);
					spp.solvePanoParameter_ceres2(pp, pd2);
//					spp.solvePanoParameter_ceres3(pp, pd2);
			...
			for (int k = j + 1; k < nPts; k++)
			{
				...
				for (int l = k + 1; l < nPts; l++)
				{
					...
					for (int m = l + 1; m < nPts; m++)
					{
						...
						SPP_S spp(8192, 4096);
						spp.solvePanoParameter_ceres(pp, pd2);
